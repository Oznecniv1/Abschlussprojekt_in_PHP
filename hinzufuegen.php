                    ...
                    try{
                        ...
                        # Tabelle Rezept !Funktioniert! FINGER WEG!
                            $query = "INSERT INTO rezepte(rez_id,rezeptname,zubereitung,users_id,bild) 
                                        VALUES (NULL,'{$formdata['rezeptname']}','{$formdata['zubereitung']}',users_id,bild)";
                                mysqli_query($db,$query) or die("Fehler beim Datenschreiben 1".mysqli_error($db));
                                $rez_id = mysqli_insert_id($db);
                        
                        # Tabelle zutaten !Funktioniert! FINGER WEG!
                        foreach($formdata['zutatenname'] as $value){
                            // if(){
                            //     $query ="SELECT * FROM zutatenname";
                            // }
                            // else{
                            $query = "INSERT INTO zutaten(zut_id,zutatenname) 
                                        VALUES (NULL,'$value')";
                                mysqli_query($db,$query) or die("Fehler beim Datenschreiben 2".mysqli_error($db));
                                array_push($zut_id, mysqli_insert_id($db));
                                // }
                            }
                        # Tabelle reztag !Funktioniert! FINGER WEG!
                        foreach($formdata['tag'] as $value){
                            $query = "INSERT INTO reztag(rez_id,tag_id) 
                                    VALUES ('$rez_id','$value')";
                                mysqli_query($db,$query) or die("Fehler beim Datenschreiben 3".mysqli_error($db));
                            }  
                        # Tabelle rezzut !Funktioniert! FINGER WEG!
                        //foreach($formdata['menge'] as $value){
                            $i=0;
                            while($i<sizeof($formdata['menge'])){//dreckige lösung von till
                            $query = "INSERT INTO rezzut(rez_id,zut_id,menge,ein_id) 
                                        VALUES ($rez_id,$zut_id[$i],{$formdata['menge'][$i]},{$formdata['einheit'][$i]})";
                                mysqli_query($db,$query) or die("Fehler beim Datenschreiben 3".mysqli_error($db));
                                $i++;
                            }
                            mysqli_commit($db);
